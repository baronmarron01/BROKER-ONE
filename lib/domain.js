...
export function classifyRequest(request) {
  const isRegulatedSector = request.regulated_override ?? REGULATED.has(request.category);
...
export function scoreCandidate(request, candidate) {
  ...
  const price = Number(candidate.total_price);
  let priceScore = 0.5;
  ...
  const eligible = constraints > 0 && candidate.eligible !== false;
  return {
    ...
    explanation: eligible ? 'Eligible; ranked by relevance, total-price fit, location, capability and evidence confidence.' : 'Excluded because a mandatory constraint was not met.'
  };
}
