...
export function planDiscoveryQueries(request) {
  ...
  return [...new Set(phrases.map(term => boundedQuery(`${term}${location ? ` ${location}` : ''}`)))].slice(0, MAX_DISCOVERY_QUERIES);
}
...
export async function searchGleif(request, limit = 5) {
  ...
  const settled = await Promise.allSettled(queries.map(async query => {
    ...
    const payload = await fetchJson(url, { timeout:2500, headers: { 'User-Agent': 'BROKER-ONE/2.5 supplier-discovery' } });
...
export async function searchWikidata(request, limit = 8) {
  const queries = planDiscoveryQueries(request);
  const settled = await Promise.allSettled(queries.map(async (query, index) => {
    const language = index % 2 === 0 ? 'fr' : 'en';
    ...
    const payload = await fetchJson(url, { timeout:2500, headers: { 'User-Agent': 'BROKER-ONE/2.5 supplier-discovery contact=admin@broker-one.invalid' } });
